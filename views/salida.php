      ...
      <section class="container">
        <div class="row justify-content-between">
          <div class="column__menu col-md-3">
            ...
            <form method="post">
                <?php
                  include("../controlador/crear_salida.php");
                ?>
              <div class="mb-2">
                <label for="exampleInputEmail1" class="form-label"
                  >Fecha salida</label
                >
                <input type="date" class="form-control" name="fechaSalida" autofocus/>
              </div>
              <div class="mb-2">
                <label for="exampleInputPassword1" class="form-label"
                  >Valor salida</label
                >
                <input type="number" class="form-control" name="valSalida" />
              </div>
              <div class="mb-2">
                <label for="exampleInputPassword1" class="form-label"
                  >Cantidad salida</label
                >
                <input type="number" class="form-control" name="cantSalida" />
              </div>
              <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label"
                  >Descuento</label
                >
                <input type="number" class="form-control" name="descSal" />
              </div>
              <div class="mb-3">
                <select class="form-select" name="selIns">
                  <option selected>Insumo a vender</option>
                  <?php
              ...
              <div class="mb-4">
                <select class="form-select" name="cliente">
                  ...
                  <?php
                        ...
                        { 
                    ?> 
                          <option value="<?php echo $datos['idcliente']?>" ><?php echo $datos['nombre_cliente']?></option>                   
                        <?php }    
                        ?>            
                </select>
              </div>
          ...
          <div class="column__menu column__menu2 col-md-8">
            <form class="d-flex" role="search">
              <input
                class="form-control me-2" id="busqueda"
                type="search"
                placeholder="&#128270;"
              />
              <button id="btn-search" class="btn btn-success" type="submit">Buscar</button>
            </form>
            <div class="table-responsive">
              <table id="tabla" class="table table-striped">
                ...
                <tbody>
                <?php
                    include("../controlador/listar_salida.php");
                    while($datos=mysqli_fetch_array($query)){ ?> 
                      <tr>
                        <td><?= $datos['idSalida_insumo'] ?></td>
                        <td><?= $datos['fecha_salida'] ?></td>
                        <td><?= $datos['cantidad_salida'] ?></td>
                        <td><?= $datos['valor_salida'] ?></td>
                        <td><?= $datos['descuento'] ?></td>
                        <td><?= $datos['nombre_cliente'] ?></td>
                        <td><?= $datos['nombre_insumo'] ?></td>
                        <td>
                          <div class="btn-group btn-group-sm" role="group">
                             <a href="../controlador/actualizar_cliente.php?idSalida_insumo=<?=$datos['idSalida_insumo'] ?>"><button type="button" style="background-color:#e69b00;border:none; padding:5px;"><i class="bi bi-pencil-square" style="color: #000000;"></i></button></a>
                          </div>
                        </td>
                      </tr>
                    <?php }    
