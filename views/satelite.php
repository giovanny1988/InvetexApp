      ...
      <section class="container">
        <div class="row justify-content-between">
          <div class="column__menu col-md-3">
            <h5>Registrar satélite</h5>
            <br />
            <form method="post">
                <?php
                    include("../controlador/crear_satelite.php");
                ?>
                <div class="mb-2">
                  <label for="cliente" class="form-label"
                    >Nombre satélite</label
                  >
                  <input type="text" class="form-control" autofocus name="satelite" />
                </div>
                <div class="mb-2">
                  <label for="correoCliente" class="form-label"
                    >Correo electrónico</label
                  >
                  <input type="email" class="form-control" name="correoSat" />
                </div>
                <div class="mb-3">
                  <label for="dirCliente" class="form-label"
                    >Dirección</label
                  >
                  <input type="text" class="form-control" name="dirSat" />
                </div>
                <div class="mb-2">
                  <label for="telCliente" class="form-label"
                    >Telefono de contacto</label
                  >
                  <input type="tel" class="form-control" name="telSat" />
                </div>
          ...
          <div class="column__menu column__menu2 col-md-8">
            ...
            <div class="table-responsive">
              <table id="tabla" class="table table-striped">
                ...
                <tbody>
                <?php
                    include("../controlador/listar_satelite.php");
                    while($datos=mysqli_fetch_array($query)){ ?> 
                      <tr>
                        <td><?= $datos['idSatelite'] ?></td>
                        <td><?= $datos['nombre_satelite'] ?></td>
                        <td><?= $datos['correo_satelite'] ?></td>
                        <td><?= $datos['direccion_satelite'] ?></td>
                        <td><?= $datos['telefono_satelite'] ?></td>
                        <td>
                          <div class="btn-group btn-group-sm" role="group">
                            <a href="../controlador/actualizar_cliente.php?idSatelite=<?=$datos['idSatelite'] ?>"><button type="button" style="background-color:#e69b00;border:none; padding:5px;"><i class="bi bi-pencil-square" style="color: #000000;"></i></button></a>
                          </div>
                        </td>
                      </tr>
                    <?php }    
